...
class IPaging
{
	...
	public function getPageBar($url='', $attrs='')
	{
		...
		if($this->index>1)$tem.="<a href='{$href}' {$attr}>上一页</a>";
        ...
        $attr = str_replace('[page]',$this->getIndex()+1,$attrs);
        $href = $baseUrl.($flag?$this->getIndex()+1:'');
		if($this->index<$this->totalpage)$tem.="<a href='{$href}' {$attr}>下一页</a>";
	...
	public function front_getPageBar($url='', $attrs='')
	{
        $attr = '';
        if($attrs != '')
        {
        ...
        $attr = str_replace('[page]',1,$attrs);
        $href = $baseUrl.($flag?1:'');
		$tem="<div class='page'><a href='{$href}' {$attr}>首页</a>&nbsp";

        $attr = str_replace('[page]',$this->getIndex()-1,$attrs);
        $href = $baseUrl.($flag?$this->getIndex()-1:'');
		if($this->index > 1)$tem.="<a href='{$href}' {$attr}>上一页</a>&nbsp";
		...
		for($i=$this->firstpage;$i<=$this->lastpage;$i++)
		{
			...
			if($i==$this->index)
			{
				$tem.="<a class='curr' href='{$href}' {$attr}>{$i}</a>&nbsp";
			}
			else
			{
				$tem.="<a href='{$href}' {$attr}>{$i}</a>&nbsp";
			}
		}

        $attr = str_replace('[page]',$this->getIndex()+1,$attrs);
        $href = $baseUrl.($flag?$this->getIndex()+1:'');
		if($this->index < $this->totalpage)$tem.="<a href='{$href}' {$attr}>下一页</a>&nbsp";
		if($this->totalpage==0)$this->index=1;
		$attr = str_replace('[page]',$this->totalpage,$attrs);
		$href = $baseUrl.($flag?$this->totalpage:'');
		return $tem."<a href='{$href}' {$attr}>末页</a>&nbsp<span>共{$this->totalpage}页</span></div>";
	}
}
?>
