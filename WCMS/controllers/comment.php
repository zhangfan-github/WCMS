...
{
	protected $checkRight  = array(
"suggestion_list","suggestion_edit","suggestion_edit_act","suggestion_del","comment_list","comment_edit","comment_del","discussion_list","discussion_edit","discussion_del","refer_list","refer_del","refer_edit","refer_reply","message_list","message_del","message_send");
    public $layout='admin';
	private $data = array();
	 ...
	 * @brief 筛选咨询信息
	 */
	function refer_filter()
	{
		$username = IReq::get('username');
		$goodsname = IReq::get('goodsname');
		$beginTime = IReq::get('beginTime');
		...
		if($goodsname)
		{
			$where .= 'goods.name like "%'.$goodsname.'%"'.$and;
		}
		if($beginTime)
		{
			$where .= 'r.time > "'.$beginTime.'"'.$and;
		}
		if($endTime)
		{
			$where .= 'r.time < "'.$endTime.'"'.$and;
		}
		$where .= ' 1 ';
		$query = new IQuery("refer as r");
		$query->join = "left join goods as goods on r.goods_id = goods.id left join user as u on r.user_id = u.id left join order as o on o.id = r.order_id left join admin as admin on r.admin_id = admin.id";
		$query->fields = "r.*,u.username,goods.name as goods_name,admin.admin_name";
		$query->page = $page;
		$query->where = $where;
		$this->data['items'] = $query->find();
		$this->data['pageBar'] = $query->getPageBar('/comment/refer_filter/');
		$this->data['filter'] = array('username'=>$username,'goodsname'=>$goodsname,'beginTime'=>$beginTime,'endTime'=>$endTime);
		$this->setRenderData($this->data);
		$this->redirect("refer_filter");
	}
