...
class PhotoUpload
{
	 ...
	 */
	function thumb($fileName,$width,$height,$extName = '_thumb')
	{
		...
		foreach($upState as $key => $rs)
		{
			...
			foreach($rs as $innerKey => $val)
			{
				if($val['flag']==1)
				{
					...
					//将图片信息入库
					$source_id = $this->insert($insertData,$photoObj);
					//将插入source 表后返回的自增量记录到$rs中
					$rs[$innerKey]['source_id']  = $source_id;
