...
class CountSum
{
	...
	private function goodsCount($buyInfo)
	{
    	...
    	if(isset($buyInfo['goods']['id']) && !empty($buyInfo['goods']['id']))
    	{
    		...
    		{
    			//优惠情况(1)
    			if(isset($discountGoodsResult[$val['id']]['discount']))
    			{
    				$goodsList[$key]['reduce'] = $val['sell_price'] - $discountGoodsResult[$val['id']]['discount'];
    			}
    	...
    	if(isset($buyInfo['product']['id']) && !empty($buyInfo['product']['id']))
    	{
    		...
    		{
    			//优惠情况(1)
    			if(isset($discountProductResult[$val['id']]['discount']))
    			{
    				$productList[$key]['reduce'] = $val['sell_price'] - $discountProductResult[$val['id']]['discount'];
    			}
    ...
    public function direct_count($id,$type,$buy_num = 1,$promo='',$active_id='')
    {
		...
		if($promo == 'time')
		{
			...
			if(!empty($promotionRow))
			{
				...
				if($promotionRow['user_group'] == 'all' || stripos(','.$promotionRow['user_group'].',',$memberRow['group_id'])!==false)
				{
